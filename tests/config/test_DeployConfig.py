...
@mock.patch("pathlib.Path.is_dir", return_value=True)
@mock.patch("os.getenv", side_effect=lambda key, default=None: "VALID_PEM" if key == "SNOWFLAKE_PRIVATE_KEY_PEM" else default)
...
@mock.patch("pathlib.Path.is_dir", return_value=True)
@mock.patch("os.getenv", side_effect=lambda key, default=None: "INVALID_PEM" if key == "SNOWFLAKE_PRIVATE_KEY_PEM" else default)
